      ...
      >
        <main className={`${isDark ? 'bg-gray-900' : 'bg-gray-50'} p-3 sm:p-4 md:p-6`}>
          <div className={`max-w-7xl mx-auto`}>
            ...
            <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} p-4 sm:p-6 md:p-8 rounded-xl shadow-lg mb-4 md:mb-6`}> 
              <h2 className={`text-xl sm:text-2xl md:text-3xl font-bold mb-3 md:mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                {t.studentHomePage.title}
              </h2>

              <p className={`${isDark ? 'text-gray-300' : 'text-gray-600'} text-sm sm:text-base md:text-lg mb-4 md:mb-6`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                ...
                <div 
                  ...
                  className={`${isDark ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gradient-to-br from-[#eaf6f6] to-white hover:from-[#d5eded] hover:to-gray-50'} p-4 sm:p-5 md:p-6 rounded-xl border-2 ${isDark ? 'border-gray-600' : 'border-[#366c6b]/20'} cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-xl group`}
                >
                  <div className={`flex items-start gap-3 sm:gap-4 ${language === 'ar' ? 'flex-row-reverse' : 'flex-row'}`}>
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-[#366c6b] to-[#1a3535] rounded-xl flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-300">
            ...
            <div className={`${isDark ? 'bg-gray-800' : 'bg-white'} p-4 sm:p-5 md:p-6 rounded-xl shadow-lg`}>
              <div className={`flex items-start gap-2 sm:gap-3 ${language === 'ar' ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg ${isDark ? 'bg-blue-900/30' : 'bg-blue-100'} flex items-center justify-center flex-shrink-0`}>
                  <svg className={`w-4 h-4 sm:w-5 sm:h-5 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} fill="currentColor" viewBox="0 0 20 20">
                ...
                <div className="flex-1">
                  ...
                  <ul className={`text-xs sm:text-sm space-y-1 ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
