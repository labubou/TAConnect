...
def send_booking_confirmation_email(student, instructor, slot, booking_date, booking_time):
    ...
    This function is sent when instructor confirms the booking.
    ...
    #?Removed this because students need to be notified of confirmed bookings
    ...
    if not student_sent:
        result = send_email(
            subject='Booking Confirmation - TA Connect',
            template_name='booking_confirmation_email_Student.html',
        ...
        if result:
            ...
            errors.append(f"Failed to send booking confirmation email to student: {student.email}")

    # Send email to instructor/TA
    if not instructor_sent:
        result = send_email(
            subject='New Booking Received - TA Connect',
            template_name='booking_confirmation_email_TA.html',
            context=email_context,
        ...
        if result:
            ...
            errors.append(f"Failed to send booking confirmation email to instructor: {instructor.email}")
